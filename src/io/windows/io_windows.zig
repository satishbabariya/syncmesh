...
pub const IoWindows = struct {
    ...
    pub fn init(allocator: Allocator, options: IO.InitOptions) !IoWindows {
        const iocp = windows.kernel32.CreateIoCompletionPort(
            ...
            null,
            0,
            0  // Use system default (number of processors)
    ...
    fn submitRead(self: *IoWindows, completion: *Completion) void {
    ...
    fn submitWrite(self: *IoWindows, completion: *Completion) void {
    ...
    fn submitAccept(self: *IoWindows, completion: *Completion) void {
        ...
        const client_socket = windows.ws2_32.accept(
            @intFromPtr(op.socket),
            op.address,
            op.address_size
        );
    ...
    fn submitConnect(self: *IoWindows, completion: *Completion) void {
        const op = completion.operation.connect;
        
        const result = windows.ws2_32.connect(
            @intFromPtr(op.socket),
            op.address,
            op.address_size
        );
    ...
    fn submitClose(self: *IoWindows, completion: *Completion) void {
    ...
    fn submitFsync(self: *IoWindows, completion: *Completion) void {
