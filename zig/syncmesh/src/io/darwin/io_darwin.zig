...
pub const IoDarwin = struct {
    ...
    fn submitClose(_: *IoDarwin, completion: *Completion) void {
    ...
    fn submitTimeout(self: *IoDarwin, completion: *Completion) void {
        ...
        const result = darwin.kevent(self.kqueue_fd, &[_]darwin.Kevent{change}, 1, &[_]darwin.Kevent{}, 0, null);
        if (result == -1) {
            completion.callback(completion.context, completion, error.TimerAddFailed);
        }
    }

    fn submitFsync(_: *IoDarwin, completion: *Completion) void {
    ...
    fn submitOpenat(_: *IoDarwin, completion: *Completion) void {
        const op = completion.operation.openat;

        const result = std.posix.openat(op.dir_fd, op.path, std.posix.O{}, op.mode);
        if (result) |fd| {
    ...
    pub fn tick(self: *IoDarwin) !u32 {
        ...
        const dummy_changelist = &[_]darwin.Kevent{};
        const num_events = darwin.kevent(self.kqueue_fd, dummy_changelist, 0, self.events.ptr, @intCast(self.events.len), &timeout_overlay.ts);
        ...
        for (self.events[0..@intCast(num_events)]) |event| {
            ...
            switch (completion.operation) {
                ...
                .accept => {
                    const op = completion.operation.accept;
                    const result: anyerror!i32 = std.posix.accept(op.socket, op.address, op.address_size, 0);
                    if (result) |fd| {
                        const usize_fd: usize = @intCast(fd);
                        completion.callback(completion.context, completion, usize_fd);
                    } else |err| {
                        completion.callback(completion.context, completion, err);
                    }
                },
